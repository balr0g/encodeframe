...
def add(A, B):
    ...
    assert A.D == B.D
    return Mat(A.D, {(itm1, itm2):A[(itm1, itm2)] + B[(itm1, itm2)] for itm1 in A.D[0] for itm2 in A.D[1] if A[(itm1, itm2)] + B[(itm1, itm2)] != 0})

def scalar_mul(M, alpha):
    "Returns the product of scalar alpha with M" 
    return Mat(M.D, {(itm1, itm2):M[(itm1, itm2)]*alpha for itm1 in M.D[0] for itm2 in M.D[1]})
...
def equal(A, B):
    ...
    for itm1 in A.D[0]:
        for itm2 in A.D[1]:
            if A[(itm1, itm2)] != B[(itm1, itm2)]:
                return False
    return True

def transpose(M):
    "Returns the transpose of M"
    return Mat((M.D[1], M.D[0]),{(itm2, itm1):M[(itm1, itm2)] for itm1 in M.D[0] for itm2 in M.D[1]})

def vector_matrix_mul(v, M):
    "Returns the product of vector v and matrix M"
    assert M.D[0] == v.D
    return Vec(M.D[1], {itm1:sum([v[k] * M[(k, itm1)] for k in M.D[0]]) for itm1 in M.D[1]})
    
def matrix_vector_mul(M, v):
    "Returns the product of matrix M and vector v"
    assert M.D[1] == v.D
    return Vec(M.D[0], {itm1:sum([v[k] * M[(itm1, k)] for k in M.D[1]]) for itm1 in M.D[0]})

def matrix_matrix_mul(A, B):
    "Returns the product of A and B"
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), {(itm1, itm2):sum([A[itm1, j] * B[j, itm2] for j in A.D[1]]) for itm2 in B.D[1] for itm1 in A.D[0]})
